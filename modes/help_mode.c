...
void help_mode_render(uint16_t mpr_touched_state, float v_sys) {
    ...
    if (page_changed) {
        ...
        snprintf(header_string, sizeof(header_string), "HELP page: %d", g_sys_page + 1);
...
void help_mode_handle_scroll(int enc_delta) {
    if (enc_delta != 0) {
...
// СТРАНИЦА 1: диагностика геометрии SD-карты
static void SD_render_1(uint16_t touched, float v_sys, int start_y, bool page_changed) {
    char buf[40];
...
// СТРАНИЦА 2: Аппаратный маппинг Pinout (Ультра-плотный формат под 34 символа)
static void pinout_render_(int start_y) {
...
static void file_structure_render_3(int start_y) {
